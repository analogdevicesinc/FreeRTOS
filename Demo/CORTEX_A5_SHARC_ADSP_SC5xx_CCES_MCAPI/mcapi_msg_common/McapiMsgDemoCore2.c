 ...
 * Project: RTOSDemo_SC589_MCAPI_Msg_Core2
 * File:    McapiMsgDemoCore2.c
 *
...
static portTASK_FUNCTION( vMcapiDemoTask, pvParameters )
{
	...
	printf("#### Hello MCAPI testing CORE2 ####\n");

	local_ep = mcapi_endpoint_create(CORE2_PORT_NUM, &mcapi_status);
	...
	for (i = 0; i < TEST_ROUNDS; i++) {
		...
		if (MCAPI_SUCCESS == mcapi_status) {
			msg[msg_size] = '\0';
			sscanf(msg, "core0 send to core2 %d", &digit);
			snprintf(vBuffer, sizeof(vBuffer), "%s %d", CORE2_SEND_STRING, ++digit);
		}
		/* Send a message back */
		mcapi_msg_send(local_ep, remote_ep, vBuffer,
						strlen(vBuffer), 0, &mcapi_status);
		mcapiErrorCheck(mcapi_status, "msg_send", 2);
		mcapi_run_round++;
	}
	printf("Core2 Test %d rounds Passed\n", i);
